...
class RegisterApp:
    ...
    def home(self, st):
        with st.form(key='Register', clear_on_submit=False):
            ...
            if submitted:
                status = self.handle_login(user=self.__username, email=self.__email,
                                           password=self.__password1, cpf=self.__cpf, phone=self.__phone)
    ...
    def handle_login(self, user, password, email, cpf, phone):
        ...
        response = requests.post('https://btk-ai-app.herokuapp.com/auth/register/', data=json.dumps(
