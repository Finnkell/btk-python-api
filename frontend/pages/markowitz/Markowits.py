...
def plot_int(index, targetvols, targetrets, opt_sharpe, opt_var, dict_data, array_weights, returns, st):
    ...
    ax.plot(portfolio_stats(opt_sharpe['x'], returns)[1], portfolio_stats(
        opt_sharpe['x'], returns)[0], 'r*', markersize=15.0)

    # Minimum Variance Portfolio
    ax.plot(portfolio_stats(opt_var['x'], returns)[1], portfolio_stats(
        opt_var['x'], returns)[0], 'b*', markersize=15.0)
    # Fronteira Eficiente
    ax.plot(dict_data["volatilidade"][int(index)],
            dict_data["retorno"][int(index)], 'o', markersize=10.0)
    # Nosso Portifolio
    ax.plot(portfolio_stats(array_weights, returns)[1], portfolio_stats(
        array_weights, returns)[0], 'h', color='green', markersize=15.0)

    ax.set_xlabel('Expected Volatility')
    ax.set_ylabel('Expected Return')
    ax.grid(True)
    # VaR Paramétrico
    pVaR_90, pVaR_95, pVaR_99 = Var_Port(
        dict_data['weight'][int(index)], returns)

    # VaR Histórico
    pCVaR_90, pCVaR_95, pCVaR_99, pHVaR_90, pHVaR_95, pHVaR_99 = CVar_Port(
        dict_data['weight'][int(index)], returns)
    ...
    pesos_acao_print = [
        f"{str(np.round(i, 2))}%" for i in dict_data['weight'][int(index)]]
    st.write(f"Pesos para cada ação: {pesos_acao_print}")
    st.write("VaR Paramétrico \n90 :", np.round(pVaR_90*100, 2), "/95 :",
             np.round(pVaR_95*100, 2), "/99 :", np.round(pVaR_99*100, 2))
    st.write("VaR Histórico   \n90 :", np.round(pHVaR_90*100, 2), "/95 :",
             np.round(pHVaR_95*100, 2), "/99 :", np.round(pHVaR_99*100, 2))
    st.write("CVaR Histórico \n 90 :", np.round(pCVaR_90*100, 2), "/95 :",
             np.round(pCVaR_95*100, 2), "/99 :", np.round(pCVaR_99*100, 2))
...
def markowitz(df_symbols: pd.DataFrame, st):
    ...
    array_weights = np.array([[0.15], [0.05], [0.07], [0.15], [0.05], [
                             0.14], [0.15], [0.15], [0.05], [0.04]])
    ...
    for i in range(5000):
        ...
        vols.append(
            np.sqrt(multi_dot([weights.T, returns.cov()*252, weights])))
    ...
    opt_sharpe = sco.minimize(min_sharpe_ratio, initial_wts, args=(
        returns,), method='SLSQP', bounds=bnds, constraints=cons)
    stats = ['Returns', 'Volatility', 'Sharpe Ratio']

    opt_var = sco.minimize(min_variance, initial_wts, args=(
        returns,), method='SLSQP', bounds=bnds, constraints=cons)
    array_weights = array_weights.flatten()

    data = pd.DataFrame(
        columns=['Retorno', 'Volatilidade', 'Indice Sharpe', 'pesos'])
    ...
    for tr in targetrets:
        ...
        opt_ef = sco.minimize(min_volatility, initial_wts, args=(
            returns,), method='SLSQP', bounds=bnds, constraints=ef_cons)
    ...
    index_dict = [i for i in range(len(dict_data["retorno"])) if round(
        dict_data["retorno"][i], 4) % portfolio_stats(array_weights, returns)[0] <= 0.005]

    retorno_print = np.round(dict_data['retorno'][index_dict[0] - 1], 2)
    vol_print = np.round(dict_data['volatilidade'][index_dict[0] - 1], 2)
    weights_plot = [
        f"{str(np.round(i, 2))}%" for i in dict_data['weight'][index_dict[0] - 1]]
    ...
    index_dict = wallet_reposition(
        "retorno", array_weights, dict_data, opt_sharpe, returns)

    # plot_index = st.slider('Movimento sua cartira dentro da fronteira eficiente', 0, 249, 1)
    fig = plot_int(0, targetvols, targetrets, opt_sharpe,
                   opt_var, dict_data, array_weights, returns, st)
    st.pyplot(fig)
