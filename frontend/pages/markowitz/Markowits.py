...
def wallet_reposition(reposition, array_weights, dict_data, opt_sharpe, returns):
    ...
    if reposition == "retorno": index_port = 0
    if reposition == "volatilidade": index_port = 1
    if reposition == "sharpe": 
        index_port = 2
        data = opt_sharpe['x']

    if len([i for i in range(len(dict_data[reposition])) if round(dict_data[reposition][i], 4) % portfolio_stats(data, returns)[index_port] <= 0.005]): return [i for i in range(len(dict_data[reposition])) if round(dict_data[reposition][i], 4) % portfolio_stats(array_weights, returns)[index_port] <= 0.005][0] - 1
    return None
